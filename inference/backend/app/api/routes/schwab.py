...
from app.methods.schwab_methods import (
    ...
    exchange_code_for_token_account,
...
@router.get("/callback-account", response_class=HTMLResponse)
async def schwab_callback_account(request: Request):
    """
    Handle the callback from Schwab after user authorization.
    """
    try:
        ...
        auth_code = request.query_params.get('code')
        if not auth_code:
            raise HTTPException(status_code=400, detail="Authorization code not found in the request for account update.")

        # Exchange the authorization code for an access token
        schwab_config = get_schwab_config()
        token_response = exchange_code_for_token_account(auth_code, schwab_config)
        if not token_response:
            raise HTTPException(status_code=500, detail="Failed to exchange authorization code for access token for account update.")
        access_token = token_response.get("access_token")
        refresh_token = token_response.get("refresh_token")
        if not access_token or not refresh_token:
            raise HTTPException(status_code=500, detail="Access token or refresh token not found in the response.")
        ...
        # Update the configuration with the new tokens
        update_schwab_config("access_token_account", access_token)
        update_schwab_config("refresh_token_account", refresh_token)

        # Return a success message with the token information
        return HTMLResponse(content=f"Authorization successful!<br/> Access Token and refresh token updated in the configuration file for Account.<br/>")
